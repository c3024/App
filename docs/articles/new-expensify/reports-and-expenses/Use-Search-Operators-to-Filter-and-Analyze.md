...
| `paid-by:` | Filter by the member who paid the report (email, phone, display name, or `me`). Combine multiple members with commas. Reports that have not been paid never match. Also works on expense, invoice, and trip searches | `paid-by:me,alice@acme.com` |
